...
def load_checkpoint(args, backbone, head, optimizer, train_loader, p_identities, p_images):

    # resume from a checkpoint
    name = "Checkpoint_Head_{}_Backbone_{}_Dataset_{}_p_idx{}_p_img{}_Epoch_".format(args.head_name, args.backbone_name, args.name, str(p_identities), str(p_images))
    print(name)
    checkpoints_model_root = os.path.join(args.checkpoints_root, str(args.backbone_name) + '_' + str(args.head_name))
    ...
    if len(potential_checkpoints) !=0:
        ...
        epoch = 0
        batch = 0
    return backbone, head, optimizer, epoch, batch, checkpoints_model_root
