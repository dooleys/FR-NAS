...
if __name__ == '__main__':
    ...
    dataloader_val = dataloaders['test']
    ...
    model, model_ema, optimizer, epoch, batch, checkpoints_model_root = load_checkpoint(
        args, model, model_ema, optimizer, dataloader_train, p_identities,
        p_images)
    #model = nn.DataParallel(model)
    model = model.to(device)
    #print(model_ema)
    
    out_file = os.path.join(args.RFW_checkpoints_root,"Checkpoint_Head_{}_Backbone_{}_Opt_{}_Dataset_{}_Epoch_{}_val.pkl".format(args.head, args.backbone, args.opt, args.name, str(epoch+1)))
...
#                                                         demographic_to_labels = demographic_to_labels_train, test = False)
    ...
    if comp_rank:
        ...
        pickle_file = os.path.join(args.RFW_checkpoints_root,
