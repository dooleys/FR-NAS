...
if __name__ == "__main__":
    ...
    scheduler = MOASHA(
        ...
        time_attr="step",
        mode=["min", "min"],
        metrics=["rev_acc", "disparity"],
        config_space=config_space,
    )
    trial_backend = LocalBackend(entry_point=str(entry_point),num_gpus_per_trial=4)

    stop_criterion = StoppingCriterion(max_wallclock_time=10000000000000000000000000000000000)
    tuner = Tuner(
