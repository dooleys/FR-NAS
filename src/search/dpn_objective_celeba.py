...
def fairness_objective_dpn_celeba(config, seed, budget):
    ...
    directory = "Checkpoints/Checkpoints_Layers_{}_LR_{}_Head_{}_Optimizer_{}/".format(str(str(config["edge1"])+str(
        config["edge2"])+str(config["edge3"])), config["lr"], config["head"], config["optimizer"])
    ...
    df = {"ids": list(indices_all_val), "rank_by_id": list(np.array(
        rank_val[:, 1].cpu().detach().numpy())), "gender_expression": list(demographic_all_val)}
