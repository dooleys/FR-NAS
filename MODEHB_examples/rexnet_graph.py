...
from timm.models.layers import ClassifierHead, create_act_layer, ConvNormAct, DropPath, make_divisible, SEModule
...
def _block_cfg(width_mult=1.0, depth_mult=1.0, initial_chs=16, final_chs=180, se_ratio=0., ch_div=1,layers=[1, 2, 2, 3, 3, 5]):
    strides = [1, 2, 2, 2, 1, 2]
...
class ReXNetV1(nn.Module):
    def __init__(
        ...
        ch_div=1, act_layer='swish', dw_act_layer='relu6', drop_rate=0.2, drop_path_rate=0.,layers=[1, 2, 2, 3, 3, 5]
    ):
        ...
        block_cfg = _block_cfg(width_mult, depth_mult, initial_chs, final_chs, se_ratio, ch_div, layers)
...
'''if __name__ == "__main__":
    ...
    layer1 = CSH.OrdinalHyperparameter("layer1", sequence=[1,2,3,4,5], default_value=1)
    layer2 = CSH.OrdinalHyperparameter("layer2", sequence=[1,2,3,4,5], default_value=2)
    layer3 = CSH.OrdinalHyperparameter("layer3", sequence=[1,2,3,4,5], default_value=2)
    layer4 = CSH.OrdinalHyperparameter("layer4", sequence=[1,2,3,4,5], default_value=3)
    layer5 = CSH.OrdinalHyperparameter("layer5", sequence=[1,2,3,4,5], default_value=3)
    layer6 = CSH.OrdinalHyperparameter("layer6", sequence=[1,2,3,4,5], default_value=5)

    cs.add_hyperparameters([width_mult, ch_div,layer1,layer2,layer3,layer4,layer5,layer6])
    count=0
    while count!=3:
     cfg=cs.sample_configuration()
     layers=[cfg['layer1'],cfg['layer2'],cfg['layer3'],cfg['layer4'],cfg['layer5'],cfg['layer6']]
     print(cfg['width_mult'])
     print(cfg['ch_div'])
     print(layers)
     network=ReXNetV1(num_classes=0,width_mult=cfg['width_mult'],ch_div=cfg['ch_div'],layers=layers,pretrained=False)
