...
import pickle
import time
#from dpn_graph import DPNSearchSpace
...
from fairness_objective_dpn import fairness_objective_dpn
...
def get_configspace(seed=None):
    cs = CS.ConfigurationSpace(seed)
    edge1 = CSH.OrdinalHyperparameter("edge1", sequence=[2], default_value=2)
    edge2 = CSH.OrdinalHyperparameter("edge2", sequence=[0,1,2,3], default_value=0)
    edge3 = CSH.OrdinalHyperparameter("edge3", sequence=[1], default_value=1)
    edge4 = CSH.OrdinalHyperparameter("edge4", sequence=[0,1,2,3], default_value=0)
    edge5 = CSH.OrdinalHyperparameter("edge5", sequence=[0,1,2,3], default_value=0)
    edge6 = CSH.OrdinalHyperparameter("edge6", sequence=[2], default_value=2)
    lr = CSH.UniformFloatHyperparameter("lr", lower=1e-4, upper=0.1, log=True, default_value=1e-3)
    optimizer = CSH.OrdinalHyperparameter("optimizer", sequence=["Adam","AdamW","SGD"], default_value="Adam")
    head = CSH.OrdinalHyperparameter("head", sequence=["CosFace","ArcFace","MagFace"], default_value="CosFace")
    cs.add_hyperparameters([edge1,edge2,edge3,edge4,edge5,edge6,lr,optimizer,head])
    return cs
...
def objective_function(config, budget, **kwargs):
    ...
    # Build model
    model=DPNSearchSpace()
    model.set_op_indices(edges)
    model.parse()
    ...
    num_params = np.log(np.sum(p.numel() for p in model.parameters()))
    #print(num_params.shape)
    #print(loss.shape)
    # dict representation that DEHB requires
    res = {
        "fitness": [loss.detach().numpy(), num_params],
        "cost": cost,
...
def input_arguments():
    ...
    parser.add_argument('--output_path', type=str, default="./pytorch_dpn_modehb",
...
def main():
    ...
    modehb = MODEHB(objective_function=fairness_objective_dpn, cs=cs, dimensions=dimensions, min_budget=args.min_budget,
